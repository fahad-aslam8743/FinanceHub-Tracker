...
const TransactionList = ({ onEdit }) => {
  ...
  return (
    <div className="w-full bg-white dark:bg-slate-900 transition-colors">
      <div className="p-4 md:p-6 border-b border-slate-50 dark:border-slate-800 space-y-4">
        <div className="relative group">
          ...
          {searchQuery && (
            <button onClick={() => setSearchQuery('')} className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-300 dark:text-slate-600 hover:text-slate-600 dark:hover:text-slate-300">
              <X size={14} />
            </button>
          )}
        </div>
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-1 bg-slate-100/50 dark:bg-slate-800/50 p-1 rounded-xl border border-slate-100 dark:border-slate-800">
      ...
      <div className="divide-y-0 sm:divide-y divide-slate-50 dark:divide-slate-800 min-h-[300px]">
        {filteredData?.map((t) => {
          const isIncome = t.amount > 0;
          return (
            <div 
              key={t.id}
              className="group flex flex-col sm:flex-row sm:items-center justify-between bg-white dark:bg-slate-900 px-4 md:px-8 py-5 transition-all hover:bg-slate-50/50 dark:hover:bg-slate-800/30">
              <div className="flex items-center gap-4 mb-3 sm:mb-0">
                ...
                <div className="min-w-0">
                  <p className="text-sm font-bold text-slate-800 dark:text-slate-200 truncate capitalize leading-tight">
                    {t.name}
                  </p>
              ...
              <div className="flex items-center justify-between sm:justify-end gap-6 md:gap-10 border-t sm:border-0 border-slate-50 dark:border-slate-800 pt-3 sm:pt-0">
                <p className={`text-base font-black tracking-tighter ${isIncome ? 'text-emerald-600' : 'text-slate-900 dark:text-slate-100'}`}>
        ...
        {filteredData?.length === 0 && (
          <div className="flex flex-col items-center justify-center py-24 px-4 text-center">
            <div className="w-16 h-16 bg-slate-50 dark:bg-slate-800 rounded-full flex items-center justify-center mb-4 border border-slate-100 dark:border-slate-700">
              <Search className="text-slate-200 dark:text-slate-600" size={24} />
            </div>
            <p className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em]">No records found</p>
            <button 
              onClick={() => {setSearchQuery(''); setFilter('all');}} 
              className="mt-4 text-[10px] font-black text-indigo-500 dark:text-indigo-400 uppercase tracking-widest hover:underline">
              Clear All Filters
            </button>
          </div>
        )}
